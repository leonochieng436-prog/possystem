  ...
  return (
    <div className="space-y-7">
      <div className="border-b border-border pb-5">
        ...
        <p className="mt-2 max-w-2xl text-sm text-muted-foreground">Manage live business configuration, access control, locations, financial rules, and security from one workspace.</p>
      </div>
      ...
      <Card id="business"><CardHeader><CardTitle className="flex items-center gap-2"><Building2 size={17} className="text-primary" /> Business profile</CardTitle></CardHeader><CardContent><BusinessProfileForm profile={organization} /></CardContent></Card>
      <Card id="locations"><CardHeader><CardTitle className="flex items-center gap-2"><Warehouse size={17} className="text-primary" /> Warehouses</CardTitle></CardHeader><CardContent><div className="divide-y divide-border">{warehouses.map((warehouse) => <div key={warehouse.id} className="flex items-center justify-between py-3 first:pt-0 last:pb-0"><div><p className="text-sm font-medium">{warehouse.name}</p><p className="text-[12px] text-muted-foreground">{warehouse.branch.name}</p></div><span className="text-[12px] text-success">Active</span></div>)}</div></CardContent></Card>
      <Card id="registers"><CardHeader><CardTitle className="flex items-center gap-2"><CreditCard size={17} className="text-primary" /> Registers</CardTitle></CardHeader><CardContent><div className="divide-y divide-border">{registers.map((register) => <div key={register.id} className="flex items-center justify-between py-3 first:pt-0 last:pb-0"><div><p className="text-sm font-medium">{register.name}</p><p className="text-[12px] text-muted-foreground">{register.branch.name}</p></div><span className={register.isActive ? "text-[12px] text-success" : "text-[12px] text-muted-foreground"}>{register.isActive ? "Active" : "Inactive"}</span></div>)}</div></CardContent></Card>
      <Card id="roles"><CardHeader><CardTitle className="flex items-center gap-2"><KeyRound size={17} className="text-primary" /> Roles & permissions</CardTitle></CardHeader><CardContent><div className="grid gap-3 sm:grid-cols-2">{roles.map((role) => <div key={role.id} className="rounded-[var(--radius-sm)] border border-border px-3 py-3"><p className="text-sm font-medium">{role.name}</p><p className="mt-1 text-[12px] text-muted-foreground">{role.permissions.length} permissions · {role.isSystem ? "System role" : "Custom role"}</p></div>)}</div></CardContent></Card>
      <div id="security" className="grid gap-6 xl:grid-cols-2"><Card><CardHeader><CardTitle className="flex items-center gap-2"><ShieldCheck size={17} className="text-primary" /> Active sessions</CardTitle></CardHeader><CardContent><p className="mb-3 text-[12px] text-muted-foreground">Current sessions in this organization: {sessions.length}</p>{sessions.slice(0, 5).map((session) => <div key={session.id} className="flex justify-between border-t border-border py-3"><div><p className="text-sm font-medium">{session.user.name}</p><p className="text-[12px] text-muted-foreground">{session.userAgent ?? "Unknown device"}</p></div><p className="text-[12px] text-muted-foreground">{session.createdAt.toLocaleDateString("en-KE")}</p></div>)}</CardContent></Card><Card id="taxes"><CardHeader><CardTitle className="flex items-center gap-2"><Receipt size={17} className="text-primary" /> Tax rates</CardTitle></CardHeader><CardContent>{taxRates.map((tax) => <div key={tax.id} className="flex justify-between border-t border-border py-3 first:border-0 first:pt-0"><span className="text-sm">{tax.name}</span><span className="text-sm font-semibold font-tabular">{tax.rate.toString()}%</span></div>)}</CardContent></Card></div>
      ...
      <Card id="receipts"><CardHeader><CardTitle className="flex items-center gap-2"><FileText size={17} className="text-primary" /> Receipts & invoices</CardTitle><p className="text-[12px] text-muted-foreground">These preferences are used by receipt and invoice rendering.</p></CardHeader><CardContent><ReceiptSettingsForm settings={receiptSettings} /></CardContent></Card>
      <Card id="notifications"><CardHeader><CardTitle>Notifications</CardTitle><p className="text-[12px] text-muted-foreground">Choose where operational alerts are delivered. Email delivery requires an email provider.</p></CardHeader><CardContent><NotificationSettingsForm settings={notificationSettings} /></CardContent></Card>
      <Card id="integrations"><CardHeader><CardTitle>Integrations</CardTitle></CardHeader><CardContent><p className="text-sm text-muted-foreground">Stored payment provider configurations are shown above. External email, SMS, storage, and WhatsApp integrations are not configured.</p></CardContent></Card>
      <Card id="audit"><CardHeader><CardTitle className="flex items-center gap-2"><FileText size={17} className="text-primary" /> Recent audit activity</CardTitle></CardHeader><CardContent>{auditLogs.map((log) => <div key={log.id} className="flex justify-between border-t border-border py-3 first:border-0 first:pt-0"><div><p className="text-sm font-medium">{log.action.replaceAll("_", " ")}</p><p className="text-[12px] text-muted-foreground">{log.user?.name ?? "System"} · {log.entityType}</p></div><span className="text-[12px] text-muted-foreground">{log.createdAt.toLocaleDateString("en-KE")}</span></div>)}</CardContent></Card>
      <Card id="data"><CardHeader><CardTitle>Data & backup</CardTitle></CardHeader><CardContent><p className="text-sm text-muted-foreground">Business data remains tenant-scoped and auditable. Automated backup status and export jobs require deployment storage and a backup provider.</p></CardContent></Card>
    </div>
  );
}