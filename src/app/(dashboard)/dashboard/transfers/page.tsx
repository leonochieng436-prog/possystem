...
export default async function TransfersPage() {
  ...
  const [branches, warehouses, variants] = await Promise.all([ctx.db.branch.findMany({ where: { isActive: true }, orderBy: { name: "asc" } }), ctx.db.warehouse.findMany({ where: { isActive: true }, orderBy: { name: "asc" } }), ctx.db.productVariant.findMany({ where: { isActive: true, product: { isActive: true, organizationId: ctx.organizationId } }, include: { product: true }, orderBy: { product: { name: "asc" } } })]);
