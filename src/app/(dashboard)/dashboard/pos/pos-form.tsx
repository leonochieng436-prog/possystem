...
export function PosForm({ branches, warehouses, registers, variants, customers }: { branches: Option[]; warehouses: Option[]; registers: Option[]; variants: Variant[]; customers: Option[] }) {
  ...
  const [warehouseId, setWarehouseId] = useState(warehouses.find((item) => item.branchId === (branches[0]?.id ?? ""))?.id ?? "");
  ...
  function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const payload = { branchId, registerId: String(data.get("registerId") || ""), warehouseId, customerId, paymentMethod, amountPaid: amountPaid || total.toString(), payments: splitPayment ? [{ method: paymentMethod, amount: amountPaid || "0" }, { method: secondPaymentMethod, amount: secondPaymentAmount || "0" }] : undefined, items: cart.map((line) => ({ variantId: line.variantId, quantity: String(line.quantity) })) };
    ...
    <form onSubmit={submit} className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_390px]">
      ...
      <aside className="sticky top-5 rounded-[var(--radius-lg)] border border-border bg-surface shadow-[0_8px_24px_rgba(18,23,26,0.06)]">
        ...
        <div className="border-t border-border px-5 py-4"><div className="space-y-2 text-sm"><div className="flex justify-between text-muted-foreground"><span>Subtotal</span><span className="font-tabular">KES {total.toFixed(2)}</span></div><div className="flex justify-between text-muted-foreground"><span>Tax</span><span className="font-tabular">KES 0.00</span></div><div className="mt-3 flex items-end justify-between border-t border-border pt-3"><span className="font-semibold">Total</span><span className="font-tabular text-2xl font-bold text-primary">KES {total.toFixed(2)}</span></div></div>
          <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-1"><select name="registerId" required className="h-9 rounded border border-border-strong bg-surface px-2 text-sm"><option value="">Register...</option>{visibleRegisters.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select name="warehouseId" value={warehouseId} onChange={(event) => { setWarehouseId(event.target.value); setCart([]); }} required className="h-9 rounded border border-border-strong bg-surface px-2 text-sm"><option value="">Warehouse...</option>{visibleWarehouses.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select value={branchId} onChange={(event) => { const nextBranchId = event.target.value; setBranchId(nextBranchId); setWarehouseId(warehouses.find((item) => item.branchId === nextBranchId)?.id ?? ""); setCart([]); }} className="h-9 rounded border border-border-strong bg-surface px-2 text-sm"><option value="">Branch...</option>{branches.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select value={customerId} onChange={(event) => setCustomerId(event.target.value)} className="h-9 rounded border border-border-strong bg-surface px-2 text-sm"><option value="">Walk-in customer</option>{customers.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div>
